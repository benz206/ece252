...
> [!warning] Missing: what the OS does (lecture pp. 1–2)
> The OS sits between applications and hardware: it hides hardware details, allocates resources like CPU time and memory, resolves conflicts, and helps programs cooperate. Systems programming operates above the kernel and uses OS services for operations programs cannot perform directly.
...
> [!error] Concurrency vs. parallelism (lecture p. 2)
> **Concurrency** means multiple actions are **in progress** at once. **Parallelism** means multiple actions are **executing simultaneously**; they do not need to start or finish together.
...
```
- Remember that we can also use an arrow operator for convenience when  dereferencing a pointer.
```c
