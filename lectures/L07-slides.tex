...
\begin{frame}
	...
	By convention, ports with numbers below 1024 are considered to be reserved for system services.
...
\begin{frame}[fragile]
	...
	The function is prototyped in \texttt{netdb.h}:
...
\end{lstlisting}
	...
	Assuming that all went well, the \texttt{serverinfo} pointer is now pointing to a linked list of \texttt{struct addrinfo}.
...
\begin{frame}
	...
	The listen step is the part where we are ready for connections from a client.
...
\begin{frame}[fragile]
	...
	\texttt{listen()}: in this step we mark the socket ready.
