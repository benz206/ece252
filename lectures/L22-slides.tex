...
\begin{frame}
	...
	When we take a single-threaded program and split it off into a multithreaded program, we may introduce a lot of errors.
...
\begin{frame}
	...
	Helgrind will categorize errors into three types:
...
\begin{frame}
	...
	It will report as an error the initial order (the first order seen is the one viewed as ``correct'') and the ``incorrect'' order that is the source of the potential problem.
