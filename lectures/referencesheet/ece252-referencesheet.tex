...
int sprintf( char * buffer, const char * fmt, ... ); /* As printf, but writes to buffer; returns chars written */
int puts( const char * string ); /* Write string, plus a newline, to stdout */
void* memcpy( void *destination, const void *source, size_t num_bytes ); /* Returns destination */
void *memset( void *mem, int value, size_t num_bytes ); /* Set memory to value, byte-wise */
size_t strlen( const char * string ); /* returns length of null-terminated string, not counting the terminator */
size_t strnlen( const char * string, size_t max_size ); /* As strlen, but returns at most max_size */
int strncmp ( const char * str1, const char * str2, size_t max_size ); /* return 0 if strings equal */
char* strerror( int errnum ); /* Text description of an error code, e.g. strerror( errno ) */
int rand( ); /* Returns a pseudo-random int between 0 and RAND_MAX */
unsigned int sleep( unsigned int seconds ); /* Suspend this program; returns seconds left if interrupted */
\end{lstlisting}
...
\begin{tabular}{l|l}
	...
	\texttt{r+} & Open for reading and writing. The stream is positioned at the beginning of the file. \\ \hline
	\texttt{w+} & Open for reading and writing. The file is created if it does not exist, otherwise it is truncated. \\ \hline
...
\begin{lstlisting}[language=C]
pid_t fork( );
pid_t getpid( ); /* The process ID of the current process */
pid_t wait( int* status );
pid_t waitpid( pid_t pid, int *status, int options ); /* 0 for options fine */
int WEXITSTATUS( int status ); /* The child's exit status */
...
int pthread_sigmask( int how, const sigset_t * set, sigset_t * old_set ); /* Use in a multithreaded program */
/* For how: SIG_BLOCK adds set to blocked signals; SIG_UNBLOCK removes; SIG_SETMASK replaces */
\end{lstlisting}
...
struct ipc_msg { /* IPC Structure can be any struct you like, as long as the first field is long */
...
int mprotect( void* address, size_t length, int prot ); /* PROT_NONE, PROT_READ, PROT_WRITE, PROT_EXEC */
...
void freeaddrinfo( struct addrinfo *res ); /* Release the list allocated by getaddrinfo */
...
struct addrinfo {
  int ai_family;             /* AF_INET for IPv4, AF_INET6 for IPv6, AF_UNSPEC for either */
  int ai_socktype;           /* SOCK_STREAM for TCP, SOCK_DGRAM for UDP */
  int ai_protocol;           /* 0 to accept any protocol */
  int ai_flags;              /* AI_PASSIVE to fill in my own IP address */
  struct sockaddr* ai_addr;  /* The address itself; cast to struct sockaddr_in* for IPv4 */
  socklen_t ai_addrlen;      /* Length, in bytes, of ai_addr */
  struct addrinfo* ai_next;  /* Next result in the list, or NULL */
};
struct sockaddr_in { /* IPv4 address; cast to struct sockaddr* when passing to bind/connect/etc. */
  sa_family_t sin_family;   /* Address family: AF_INET */
  in_port_t sin_port;       /* Port number, in network format: use htons( ) */
  struct in_addr sin_addr;  /* IPv4 address: sin_addr.s_addr, network format; INADDR_ANY for any */
};
...
CURLcode curl_easy_getinfo( CURL *curl, CURLINFO info, ... ); /* CURLINFO_RESPONSE_CODE (long*), CURLINFO_PRIVATE (char**) */
const char* curl_easy_strerror( CURLcode error ); /* Text description of the error */
...
struct CURLMsg { /* Returned by curl_multi_info_read */
  CURLMSG msg;             /* CURLMSG_DONE if this transfer is finished */
  CURL* easy_handle;       /* The easy handle this message is about */
  union {
    void* whatever;
    CURLcode result;       /* msg->data.result; CURLE_OK if the transfer succeeded */
  } data;
};
struct curl_waitfd { /* Extra file descriptors for curl_multi_wait; may be NULL/0 if none */
  curl_socket_t fd;
  short events;   /* CURL_WAIT_POLLIN, CURL_WAIT_POLLPRI, CURL_WAIT_POLLOUT */
  short revents;
};
\end{lstlisting}
\vspace{-1em}
...
\begin{tabular}{l|l|l}
	...
	\texttt{CURLOPT\_HTTPGET} & Issue HTTP GET request (this is the default, though) & \texttt{1L}\\ \hline
	\texttt{CURLOPT\_HEADER} & Include the HTTP response header in the body output & \texttt{1L}\\ \hline
	\texttt{CURLOPT\_VERBOSE} & Print detailed information about the transfer & \texttt{1L}\\ \hline
	\texttt{CURLOPT\_PRIVATE} & Store a pointer with the handle; read it back with \texttt{CURLINFO\_PRIVATE} & \texttt{void*}\\
\end{tabular}
...
pthread_attr_init( pthread_attr_t *attributes ); /* Set the attributes to their default values */
pthread_attr_destroy( pthread_attr_t *attributes );
...
\begin{multicols}{2}
Perform the operation and return the \textit{old} value:
\begin{lstlisting}[language=C]
...
Perform the operation and return the \textit{new} value:
\begin{lstlisting}[language=C]
...
struct sigevent {
  int sigev_notify;                              /* Notify Type: SIGEV_NONE, SIGEV_SIGNAL, or SIGEV_THREAD */
...
/* For what, combine with | (bitwise OR); the callback tests them with & (bitwise AND):
   EV_TIMEOUT 0x01 timeout elapsed      EV_READ 0x02 fd ready to read     EV_WRITE  0x04 fd ready to write
   EV_SIGNAL  0x08 signal occurred      EV_ET   0x20 edge-triggered       EV_PERSIST 0x10 stay active after firing
   (without EV_PERSIST the event fires once and becomes non-pending; re-add it with event_add) */
...
struct bufferevent* bufferevent_socket_new( struct event_base* base, evutil_socket_t fd,
    enum bufferevent_options options ); /* fd = -1 to connect later; BEV_OPT_CLOSE_ON_FREE for options */
void bufferevent_free( struct bufferevent* bev );

typedef void ( *bufferevent_data_cb )( struct bufferevent* bev, void* ctx );
typedef void ( *bufferevent_event_cb )( struct bufferevent* bev, short events, void* ctx );
/* For events in the callback, test with & (bitwise AND): BEV_EVENT_CONNECTED connection succeeded,
   BEV_EVENT_ERROR an error occurred, BEV_EVENT_EOF end of file reached, BEV_EVENT_TIMEOUT timed out */
